...
public class Bonk extends SubsystemBase{
    SparkMax bonker;
    private final double endPos = -4.14;
    ...
    public Command bonk() {
        return new RunCommand(()->bonker.set(bonkPower), this).until(()->{return bonker.getEncoder().getPosition() > endPos;}).andThen(()->bonker.set(-bonkPower)).until(()->{return bonker.getEncoder().getPosition() < endPos;}).andThen(()->bonker.set(0));
    }
    @Override
